...
{
	uint8_t i;
	//每个汉字占16*16像素，即两行8*16字符
	//每个汉字占16个字节（上半部分）+16个字节（下半部分）
	uint16_t chineseIndex = 95 + Index * 3;//字库中汉字起始索引（95个ASCII字符后）
	
	//显示上半部分
	OLED_SetCursor((Line - 1) * 2, (Column - 1) * 16);
	for(i = 0; i < 16; i++)
	{
		OLED_WriteData(OLED_F8x16[chineseIndex][i]);
	}
	...
	for(i = 0; i < 16; i++)
	{
		OLED_WriteData(OLED_F8x16[chineseIndex + 1][i]);
	}
}
